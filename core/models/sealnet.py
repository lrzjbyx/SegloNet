...
class SealNet(nn.Module):
    ...
    def expand_processing(self,pairs):
        ...
        for pair in pairs:
            ...
            while not (mask_poly.union(shrink_mask_poly).area - mask_poly_area > 1) and n < self.expand_step:
                ...
                cv2.fillPoly(canvas, [np.array(shrink_mask_poly.exterior.coords, dtype=np.int32)], color=(0, 255, 0))

                # cv2.imwrite("expension-{0}.png".format(n),canvas)
    ...
    def generate_entity(self,image,pts):
        ...
        ###########################################
        ###########################################
        # cv2.imwrite("dwadsdawdadw-{0}.png".format(time.time()),image)
        return image
    ...
    def forward(self, instances , targets):
        ...
        if self.training:
            ...
            # 绘制热力图
            # for ii in range(instances["heatmap"].shape[0]):
            #     plt.rcParams['font.size'] = 16
            #     mean_tensor = torch.mean(instances["heatmap"][ii], dim=0).cpu().numpy()*10
            #     plt.imshow(mean_tensor, cmap='hot', interpolation='nearest')
            #     plt.colorbar()
            #     plt.savefig('heatmap-{0}.png'.format(ii), bbox_inches='tight', pad_inches=0, transparent=True)
            #     # plt.show()
            #     plt.close()
                # 使用matplotlib绘制热力图
                # fig, ax = plt.subplots()
                # cax = ax.imshow(mean_tensor, cmap='hot', interpolation='nearest')
                # # cax.colorbar()
                # # 移除坐标轴
                # # ax.axis('off')
                #
                ...
                #
                # # 为了确保图像没有留白，我们需要紧密地保存
                # plt.savefig('heatmap-{0}.png'.format(ii), bbox_inches='tight', pad_inches=0, transparent=True)
                # plt.close(fig)
            #

            # 绘制热力图
            # for ii in range(out_instances.shape[0]):
            #     plt.rcParams['font.size'] = 16
            #     mean_tensor = torch.mean(out_instances[ii], dim=0).cpu().detach().numpy()*10
            #     plt.imshow(mean_tensor, cmap='hot', interpolation='nearest')
            #     plt.colorbar()
            #     plt.savefig('heatmapout_instances-{0}.png'.format(ii), bbox_inches='tight', pad_inches=0, transparent=True)
            #     # plt.show()
            #     plt.close()

            # torch.save(out_instances, 'tensor.pt')
            ...
            for ii in range(out_instances.shape[0]):
                plt.rcParams['font.size'] = 16
                mean_tensor = torch.mean(out_instances[ii], dim=0).cpu().numpy()*10
                plt.imshow(mean_tensor, cmap='hot', interpolation='nearest')
                plt.colorbar()
                plt.savefig('预测文本实例-predict-axis-heatmap-{0}.png'.format(ii), bbox_inches='tight', pad_inches=0, transparent=True)
                # plt.show()
                plt.close()
            ...
            for ii in range(out_instances.shape[0]):
                ...
                # 为了确保图像没有留白，我们需要紧密地保存
                plt.savefig('预测文本实例-predict-heatmap-{0}.png'.format(ii), bbox_inches='tight', pad_inches=0, transparent=True)
                plt.close(fig)


            b_list = []
            b_canvas = np.zeros((256,256,4),dtype=np.float64)
            for ii in range(out_instances.shape[0]):
                plt.rcParams['font.size'] = 16

                # a = np.zeros((256,256,3),dtype=np.uint8)
                # poly = targets[0]['resized_polygon_points'][ii]
                # cv2.fillPoly(a, [poly.astype(np.int32)], (0, 255, 0))

                a = targets[0]["resized_image"]



                # a = instances["entity"][ii].cpu().permute(1, 2, 0).numpy()


                b = torch.mean(out_instances[ii], dim=0).cpu()*10

                # 将a和b正规化到[0,1]的范围，以便正确显示
                # a = (a - a.min()) / (a.max() - a.min())
                b_normalized = (b - b.min()) / (b.max() - b.min())

                # 使用 matplotlib 转换 b_normalized 到 RGBA
                cmap = plt.get_cmap('hot')
                b_rgba = cmap(b_normalized.numpy())

                # b_canvas

                # 将黑色变为透明
                b_rgba[b_rgba[..., :3].sum(2) < 0.5] = [0, 0, 0, 0]  # 这里0.1是一个阈值，可以根据需要调整

                b_canvas = b_canvas + b_rgba

                ero = np.zeros((5,5),np.uint8)
                b_canvas = cv2.erode(b_canvas,ero,iterations=1)


                # b_list.append(b_rgba)
                # # 绘制原始图片
                # plt.imshow(a)
                #
                # # 叠加热力图
            ...
            rt = targets[0]["reverse_trans"]
            b_canvas = cv2.warpAffine(b_canvas, rt, tuple(targets[0]['raw_image_size'][2:4]),
                                    flags=cv2.INTER_LINEAR)

            a = targets[0]["raw_image"]
            plt.imshow(a)
            plt.axis('off')
            plt.imshow(b_canvas)
            plt.savefig(
                'heatmap/{0}'.format(os.path.basename(targets[0]["ann_path"])),
                bbox_inches='tight',
                pad_inches=0,
                transparent=True,
                dpi=300  # 设置DPI
...
def u2_hr_backbone(out_ch=4,base_channel=32,key_joints=5,threshold=0.5,cfg="xx"):
